...
std::shared_ptr<std::istream> ZipArchiveEntry::GetRawStream()
{
...
std::shared_ptr<std::istream> ZipArchiveEntry::GetDecompressionStream()
{
  ...
  if (!this->IsDirectory())
  {
    if (_inputStream == nullptr)
    {
      if (!_isNewOrChanged)
      {
        ...
        compressedDataStream = this->GetRawStream().get();
      }
