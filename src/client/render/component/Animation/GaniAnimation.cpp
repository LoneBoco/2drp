...
void GaniAnimation::Load(const filesystem::path& image)
{
	...
	if (auto so = m_owner.lock())
	{
		...
		if (id == 0)
		{
			auto filepath = game->Server->FileSystem.GetFilePath(fs::FileCategory::ASSETS, filename);
			if (!filepath.empty())
			{
		...
		if (auto anim = m_animation.lock())
		{
			...
			for (const auto& [index, sprite] : anim->Sprites)
			{
				...
				else log::PrintLine(log::game, "!! Could not load texture {} for gani {}.", sprite.Image, image.filename().string());
			}
		}
